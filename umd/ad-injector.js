(function (global, factory) {
  ...
  (global = global || self, global.adInjector = factory());
  ...
  var lodash__basetostring = baseToString;
  ...
  var lodash__basevalues = baseValues;
  ...
  function isFunction(value) {
    ...
    return isObject$1(value) && objToString.call(value) == funcTag;
  }
  ...
  var reIsNative$1 = RegExp('^' +
    fnToString$1.call(hasOwnProperty$2).replace(/[\\^$.*+?()[\]{}|]/g, '\\$&')
    .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') + '$'
  );
  ...
  var isArray = nativeIsArray || function(value) {
    return isObjectLike$2(value) && isLength$2(value.length) && objToString$1.call(value) == arrayTag;
  };
  ...
  function isFunction$2(value) {
    ...
    // in older versions of Chrome and Safari which return 'function' for regexes
    // and Safari 8 equivalents which return 'object' for typed array constructors.
    return isObject$3(value) && objToString$1.call(value) == funcTag$2;
  }
   ...
   * @returns {*} Returns the "length" value.
   */
  var getLength$1 = baseProperty$1('length');
  ...
  function isArrayLike$2(value) {
    return value != null && isLength$3(getLength$1(value));
  }
  ...
  function shimKeys(object) {
    ...
    while (++index < propsLength) {
      var key = props[index];
      if ((allowIndexes && isIndex$1(key, length)) || hasOwnProperty$3.call(object, key)) {
        result.push(key);
      }
    }
    return result;
  ...
  var lodash_restparam = restParam;
  ...
  /** Built-in value references. */
  var Symbol = lodash__root.Symbol;
  ...
  function assignOwnDefaults(objectValue, sourceValue, key, object) {
    return (objectValue === undefined || !hasOwnProperty$4.call(object, key))
      ? sourceValue
      : objectValue;
  }
  ...
  function baseAssign(object, source) {
    return source == null
      ? object
      : lodash__basecopy(source, lodash_keys(source), object);
  }
  ...
  function isError(value) {
    return isObjectLike$4(value) && typeof value.message == 'string' && objToString$2.call(value) == errorTag;
  }
  ...
  function template(string, options, otherOptions) {
    ...
    if (otherOptions && lodash__isiterateecall(string, options, otherOptions)) {
    ...
    var imports = assignWith(baseAssign({}, options.imports), settings.imports, assignOwnDefaults),
        importsKeys = lodash_keys(imports),
        importsValues = lodash__basevalues(imports, importsKeys);
   ...
   * }
   */
  var attempt = lodash_restparam(function(func, args) {
    try {
  ...
  var PLACEHOLDER_CLASS = '.ad-injector-placeholder';

  function defaultExtractTrackId(data) {
    return data.toString();
  }

  function defaultExtractUrl(data) {
    return data.toString();
  }

  function defaultBuildLinkUrl(_data) {
    return '';
  }

  function defaultOnImpression(_trackId, _target, _onerror) {}

  function defaultFetchAds(target, _placeholder, onsuccess, _onerror) {
    onsuccess(target, []);
  }
  ...
  function render(ads, target, templateFn, getTrackId, getUrl, buildLinkUrl) {
    var linkUrl = buildLinkUrl(ads);
    var el = target ? target : document.createElement('span');
    el.innerHTML = templateFn(_extends({
      linkUrl: linkUrl
    }, ads));
  ...
  function loadAdFrame(config) {
    var container = config.container;
    var placeholder = config.placeholder,
        extractTrackId = config.extractTrackId,
        extractUrl = config.extractUrl,
        buildLinkUrl = config.buildLinkUrl,
        fetchAds = config.fetchAds,
        onImpression = config.onImpression;
    ...
    var containerElm = cElm;
    var io = null;

    if (!isIE6()) {
      // ie7, ie8でもだめな可能性があるので、その時はObject.definePropertyのチェックをする
      // 一応polyfillはie7対応と言い張っている
      io = new IntersectionObserver(function (entreis) {
        entreis.forEach(function (entry) {
          ...
          if (trackId && !target.getAttribute('data-post-impression')) {
            ...
            onImpression(trackId, target, function (target) {
    ...
    fetchAds(containerElm, placeholder, function (target, ads) {
      document.querySelectorAll('#' + target.id + ' ' + placeholder).forEach(function (el) {
        if (el.getAttribute("data-rendered")) return;
        var ad = ads.shift();

        if (ad) {
          var templateId = el.getAttribute('data-template-id') ? el.getAttribute('data-template-id') : config.template;
          var currentTemplate = document.querySelector(templateId);
          if (!currentTemplate) throw new Error("template " + templateId + " not found.");
          var compiled = lodash_template(currentTemplate.innerHTML);
          render(ad, el, compiled, extractTrackId, extractUrl, buildLinkUrl);

          if (io) {
            io.observe(el);
          } else {
            var _trackId2 = extractTrackId(ad);

            if (_trackId2 && !el.getAttribute('data-post-impression')) {
              onImpression(_trackId2, el, function (target) {
                target.setAttribute('data-post-impression', '');
              });
            }
          }
    ...
    }, function (_err) {// prepare request error
    });
  }

  loadAdFrame.defaults = {
    defaultExtractTrackId: defaultExtractTrackId,
    defaultExtractUrl: defaultExtractUrl,
    defaultBuildLinkUrl: defaultBuildLinkUrl,
    defaultOnImpression: defaultOnImpression,
    defaultFetchAds: defaultFetchAds
  };
